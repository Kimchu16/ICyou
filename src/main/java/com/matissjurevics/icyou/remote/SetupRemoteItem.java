 ...
 * right-click a camera terminal to bind it.
 */
public class SetupRemoteItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        // --- Bind the carried camera to a terminal ---
        if (world.getBlockState(pos).getBlock() instanceof CameraTerminalBlock) {
            if (!world.isClient) {
                ...
                if (world.getBlockEntity(pos) instanceof CameraTerminalBlockEntity terminal
                    ...
                    player.sendMessage(Text.literal("Camera bound to terminal ("
                            + terminal.getCameras().size() + " linked)"), false);
                } else {
                    player.sendMessage(Text.literal(
